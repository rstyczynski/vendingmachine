...
- terraform_fqrn.tf - Aggregates FQRN maps from all modules (generated by generate_fqrn.sh)
- terraform.tfvars - Combines all *_terraform.tfvars files (generated by generate_tfvars.sh)
...
terraform_prepare.sh - Wrapper script that runs both generation scripts and terraform init. Recommended for regenerating everything after adding new apps.
...
- Run bin/terraform_prepare.sh after adding new modules or applications (regenerates both files and runs terraform init)
- Or run bin/generate_fqrn.sh and bin/generate_tfvars.sh individually if preferred
