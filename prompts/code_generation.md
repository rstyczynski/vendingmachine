...
### `bin/terraform_prepare.sh` ⭐ **RECOMMENDED**
- **Purpose**: Generate all auto-generated files and run terraform init
- **Usage**: `./bin/terraform_prepare.sh`
- **What it does**: Runs both `generate_tfvars.sh` and `generate_fqrn.sh`, then `terraform init`
...
2. Run generation script:
   ```bash
   ./bin/terraform_prepare.sh  # Regenerates both terraform.tfvars and terraform_fqrn.tf with APP3, runs terraform init
   ```

   Or run individually:
   ```bash
   ./bin/generate_tfvars.sh  # Regenerates terraform.tfvars
   ./bin/generate_fqrn.sh    # Regenerates terraform_fqrn.tf
   ```
...
The `generate_fqrn.sh` script automatically:
