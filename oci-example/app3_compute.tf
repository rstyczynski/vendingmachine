...
module "app3_compute_instances" {
  ...
  instance_fqrn = each.key # Instance FQRN is the map key (e.g., "instance://vm_demo/demo/app3_instance")
  zone = {
...
# ═══════════════════════════════════════════════════════════════
# APP3 Compute Variables
# ═══════════════════════════════════════════════════════════════

variable "app3_compute_instances" {
  description = "Map of APP3 compute instances to create, indexed by instance FQRN (e.g., instance://vm_demo/demo/app3_instance)"
  type = map(object({
...
# ═══════════════════════════════════════════════════════════════
# APP3 Compute Outputs
# ═══════════════════════════════════════════════════════════════

output "app3_compute_instances" {
  description = "APP3 Compute instance details"
  value = {
    for k, m in module.app3_compute_instances : k => {
      id         = m.id
      name       = m.name
